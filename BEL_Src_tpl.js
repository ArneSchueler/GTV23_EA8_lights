var app = (function() {
    ...
    var illumination = {
        ambientLight : [ .5, .5, .5 ],
        light : [ 
			{isOn : true, position : [ 0., 1., 3. ],color : [ 0.2, 1., 1. ], zAngle:6},
			        {isOn:true, position:[0.,1.,-3.], color:[1.,0.2,1.], zAngle:3}, ]
    };
    ...
    function initDataAndBuffers(model, geometryname) {
	    ...
	    function calculateLightOrbit(light) {
	    // Calculate x,z position for light orbiting the center.
	    var x = light.position[0];
	    var z = light.position[2];
	
	    light.position[0] = camera.center[0] + camera.distance * Math.sin(light.zAngle);
	    light.position[2] = camera.center[2] + camera.distance * Math.cos(light.zAngle);
	}
    ...
    function initEventHandler() {
        ...
        window.onkeydown = function(evt) {
            ...
            case ('Z'):
                ...
                break;
            case ('L'): // Beispiel-Taste 'L'
                // Orbit both lights.
                illumination.light.forEach(function (light) {
                    light.zAngle += deltaRotate;
                    calculateLightOrbit(light);
                });
                break;
            case ('I'): // Beispiel-Taste 'L'
                // Orbit both lights.
                illumination.light.forEach(function (light) {
                    light.zAngle -= deltaRotate;
                    calculateLightOrbit(light);
